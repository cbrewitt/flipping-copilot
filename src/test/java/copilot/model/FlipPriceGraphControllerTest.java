...
public class FlipPriceGraphControllerTest {
    ...
    public void testOneAccount() {
        ...
        List<Flip> flips = generateFlipsBetween(sixMonthsAgo, now, 10_000, List.of(ACCOUNT_ID_1));
        ...
        flips.sort(Comparator.comparing(Flip::getClosedTime).reversed().thenComparing(Flip::getId));
        ...
        for (int time : testTimes) {
            ...
            List<Flip> allFlips1 = flipManager.getPageFlips(1, flips.size());
            List<Flip> allFlips2 = expectedPage(flips, time, 1, flips.size(), ACCOUNT_ID_1);
            ...
            for (int pageSize : Arrays.asList(3, 20, 33, 50, 100)) {
                ...
                for(int page=1; page <= numPages; page++) {
                    List<Flip> pageFlips2 = expectedPage(flips, time, page, pageSize, ACCOUNT_ID_1);
                    List<Flip> pageFlips1 = flipManager.getPageFlips(page, pageSize);
    ...
    public void testMultipleAccounts() {
        ...
        int now = (int) Instant.now().getEpochSecond();
        int sixMonthsAgo = (int) Instant.now().minus(365/ 2, ChronoUnit.DAYS).getEpochSecond();
        List<Flip> flips = generateFlipsBetween(sixMonthsAgo, now, 5_000, List.of(ACCOUNT_ID_1, ACCOUNT_ID_2, ACCOUNT_ID_3));
        ...
        flips.sort(Comparator.comparing(Flip::getClosedTime).reversed().thenComparing(Comparator.comparing(Flip::getId).reversed()));
        ...
        List<Flip> flips1 = flipManager.getPageFlips(1, flips.size());
        ...
        for (Integer accountId : Arrays.asList(ACCOUNT_ID_1, ACCOUNT_ID_2, ACCOUNT_ID_3, null)) {
            ...
            for (int time : testTimes) {
                ...
                // check all flips equal
                List<Flip> allFlips1 = flipManager.getPageFlips(1, flips.size());
                List<Flip> allFlips2 = expectedPage(flips, time, 1, flips.size(), accountId);
                assertFlipListsEqual(allFlips1, allFlips2);
                ...
                for (int pageSize : Arrays.asList(3, 20, 33, 50, 100)) {
                    ...
                    for (int page = 1; page <= numPages; page++) {
                        List<Flip> pageFlips1 = flipManager.getPageFlips(page, pageSize);
                        List<Flip> pageFlips2 = expectedPage(flips, time, page, pageSize, accountId);
                        assertFlipListsEqual(pageFlips1, pageFlips2);
                    }
                }
            }
    ...
    public void verifyflipManagerStoredOrder(FlipManager flipManager) {
        for (int i =0; i < flipManager.weeks.size(); i++) {
            ...
            for (List<Flip> flips : w.accountIdToFlips.values()) {
    ...
    private List<Flip> expectedPage(List<Flip> flips, int time, int pageNumber, int pageSize, Integer accountId) {
        int toSkip = (pageNumber - 1) * pageSize;
        List<Flip> page = new ArrayList<>();
        for(Flip f : flips) {
    ...
    private void assertFlipListsEqual(List<Flip> f1, List<Flip> f2) {
        if(f1.size() != f2.size()) {
    ...
    private Stats expectedStats(List<Flip> flips, int time, Integer accountId) {
    ...
    private List<Flip> generateFlipsBetween(int start, int end, int number, List<Integer> accountIds) {

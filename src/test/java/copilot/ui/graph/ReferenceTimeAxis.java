...
class ReferenceTimeAxis {
        ...
        if (daysStep == 1) {
            if (dayTicks.length < 5) {
                ...
                for (int d : preAppend(dayTicks, minDay)) {
                    ...
                    if (tick06 < timeMax && tick06 > timeMin) { timeTicks = append(timeTicks, tick06); }
                    if (tick12 < timeMax && tick12 > timeMin) { timeTicks = append(timeTicks, tick12); }
                    if (tick18 < timeMax && tick18 > timeMin) { timeTicks = append(timeTicks, tick18); }
                }
            } else if (dayTicks.length < 10){
                // add only 12:00
                for (int d : preAppend(dayTicks, dayTicks[0] - Constants.DAY_SECONDS)) {
                    int tick12 = d + 12 * 60 * 60;    // 12:00
                    if (tick12 > timeMin && tick12 < timeMax) { timeTicks = append(timeTicks, tick12); }
                }
            }
        }
    ...
    public static YAxis calculateNumericAxis(long min, long max, long range, int maxAllowableTicks, int maxAllowableGridLines) {
        if (range <= 0 || max < min) { return new YAxis(new long[]{min}, new long[]{}); }
        ...
        if (remainder != 0) {
            ...
            if (min > Long.MAX_VALUE - adjustment) { return new YAxis(new long[]{}, new long[]{}); }

...
public class ReferenceGraphPanel extends JPanel {
    ...
    private final ReferenceGraphRenderer renderer;
    ...
    public ReferenceGraphPanel(GraphSettings configManager) {

        this.configManager = configManager; renderer = new ReferenceGraphRenderer(); zoomHandler = new ZoomHandler();
    ...
    protected void paintComponent(Graphics g) {
        ...
        renderer.drawLegend(graphics, config, pricePa, data.predictionTimes != null);
        ...
        renderer.drawGrid(graphics, config, pricePa, bounds, bounds::toY, xAxis, yAxis);
        renderer.drawGrid(graphics, config, volumePa, bounds, bounds:: toY2, xAxis, y2Axis);
        renderer.drawAxes(graphics, config, pricePa);
        renderer.drawYAxisLabels(graphics, config, pricePa, bounds::toY, yAxis, false);
        renderer.drawAxes(graphics, config, volumePa);
        renderer.drawYAxisLabels(graphics, config, volumePa, bounds::toY2, y2Axis, true);
        renderer.drawXAxisLabels(graphics, config, volumePa, bounds, xAxis);

        int pointSize = dynamicPointSize(Config.BASE_POINT_SIZE, bounds);
        renderer.drawPoints(graphics, pricePa, bounds, dataManager.lowDatapoints, config.lowColor, pointSize);
        renderer.drawPoints(graphics, pricePa, bounds, dataManager.highDatapoints, config.highColor, pointSize);
        if (config.connectPoints) {
            renderer.drawLines(graphics, pricePa, bounds, dataManager.lowDatapoints, config.lowColor, Config.NORMAL_STROKE);
            renderer.drawLines(graphics, pricePa, bounds, dataManager.highDatapoints, config.highColor, Config.NORMAL_STROKE);
        }
        renderer.drawStartPoints(graphics, pricePa, bounds, dataManager.buyPriceDataPoint(), Color.WHITE, pointSize);
        renderer.drawStartPoints(graphics, pricePa, bounds, dataManager.sellPriceDataPoint(), Color.WHITE, pointSize);
        if (config.showSuggestedPriceLines) { drawSuggestedPriceLine(graphics, config); }

        renderer.drawLines(graphics, pricePa, bounds, dataManager.predictionLowDatapoints, config.lowColor, Config.DOTTED_STROKE);
        renderer.drawLines(graphics, pricePa, bounds, dataManager.predictionHighDatapoints, config.highColor, Config.DOTTED_STROKE);
        if(data.predictionTimes != null) {
            renderer.drawPredictionIQR(graphics, config, pricePa, bounds, data.predictionTimes, data.predictionLowIQRLower, data.predictionLowIQRUpper, true);
            renderer.drawPredictionIQR(graphics, config, pricePa, bounds, data.predictionTimes, data.predictionHighIQRLower, data.predictionHighIQRUpper, false);
        }
        zoomHandler.drawButtons(graphics, pricePa, mousePosition);
        zoomHandler.drawSelectionRectangle(graphics, pricePa);

        renderer.drawVolumeBars(graphics, config, volumePa, bounds, dataManager.volumes, hoveredPoint);

        if(!dataManager.flipEntryDatapoints.isEmpty()) {
            renderer.drawTxsDatapoints(graphics, pricePa, bounds, dataManager.flipEntryDatapoints, hoveredPoint, config);
        }

        if(!dataManager.flipCloseDatapoints.isEmpty()) {
            renderer.drawTxsDatapoints(graphics, pricePa, bounds, dataManager.flipCloseDatapoints, hoveredPoint, config);
        }

...
public class Preferences {
    ...
    private final AccountPreferencesRS osrsAccountPreferences;
    ...
    public Preferences(Gson gson,
                                        ...
                                        AccountPreferencesRS osrsAccountPreferences) {
        this.gson = gson; this.executor = executor; this.osrsAccountPreferences = osrsAccountPreferences;
    ...
    public synchronized boolean isF2pOnlyMode() { return osrsAccountPreferences.get().isF2pOnlyMode(); }

    public synchronized boolean isBuyAndHold() { return osrsAccountPreferences.get().isBuyAndHold(); }

    public synchronized void setBuyAndHold(boolean buyAndHold) {
        var preferences = osrsAccountPreferences.get();
    ...
    public synchronized int getTimeframe() { return osrsAccountPreferences.get().getTimeframe(); }
    ...
    public synchronized RiskLevel getRiskLevel() { return osrsAccountPreferences.get().getRiskLevel(); }
    ...
    public synchronized Integer getReservedSlots() { return osrsAccountPreferences.get().getReservedSlots(); }

    public synchronized int getEffectiveReservedSlots() {
        Integer reservedSlots = getReservedSlots();
        if (reservedSlots != null) { return reservedSlots; }
    ...
    public synchronized Long getMinPredictedProfit() { return osrsAccountPreferences.get().getMinPredictedProfit(); }
    ...
    public synchronized Long getEffectiveDumpMinPredictedProfit() {
        if (!isReceiveDumpSuggestions()) { return null; }
    ...
    public synchronized void setBlockedItems(Set<Integer> blockedItems) {
        ...
        Consumer<ProfileSuggestionPreferences> update = (s) -> {
            ...
            toBlock.forEach(i -> {
                if(!s.blockedItemIds.contains(i)) { s.blockedItemIds.add(i); }
            });
        };
        log.debug("blocking {}, unblocking {}", toBlock, toUnblock);
        update.accept(cachedPreferences);
        executor.submit(() -> updateProfile(selectedProfile, update));
    }

    public synchronized void blockItem(int itemId) {
        Consumer<ProfileSuggestionPreferences> update = (s) -> {
            if(!s.blockedItemIds.contains(itemId)) { s.blockedItemIds.add(itemId); }
        };
        update.accept(cachedPreferences);
        executor.submit(() -> updateProfile(selectedProfile, update));
    ...
    public synchronized List<Integer> blockedItems() { return cachedPreferences.getBlockedItemIds(); }

    public synchronized boolean isDefaultProfileSelected() { return DEFAULT_PROFILE_PATH.equals(selectedProfile); }

    public synchronized String getCurrentProfile() { return toDisplayName(selectedProfile); }
    ...
    private synchronized void updateProfile(Path profile, Consumer<ProfileSuggestionPreferences> changes) {
        Path lockFile = Paths.get(profile+ ".lock"), tmpFile = Paths.get(profile+ ".tmp");
        try {

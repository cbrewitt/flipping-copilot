...
public class Offer {
    ...
    public long cashStackGpValue() {
        if (status == OfferStatus.SELL) { return (long) (amountTotal - amountTraded) * price; } else if (status == OfferStatus.BUY){
    ...
    public static Offer fromRunelite(GrandExchangeOffer runeliteOffer, int slotId) {
        var status = OfferStatus.fromRunelite(runeliteOffer.getState());

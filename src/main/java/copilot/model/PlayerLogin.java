...
public class PlayerLogin {
    ...
    public boolean isMembersWorld() { return client.getWorldType().contains(WorldType.MEMBERS); }
    ...
    public boolean isValidLoginState() { return getInvalidStateDisplayMessage() == null; }
    ...
    public String getInvalidStateDisplayMessage() {
        ...
        var unsupportedWorldType = getUnsupportedWorldType();
        if (unsupportedWorldType != null) { return unsupportedWorldType + " worlds<br>are not supported"; }
        if(client.isClientThread()) { lastIsIronman = client.getVarbitValue(Varbits.ACCOUNT_TYPE) != 0; }
        if (lastIsIronman) { return "Ironman accounts<br>are not supported"; }
        return null;
    }

    // todo: inline this method
    public Long getAccountHash() { return client.getAccountHash(); }

    public String getPlayerDisplayName() {
        long accountHash = client.getAccountHash();
        if (lastAccountHash == accountHash && cachedDisplayName != null) { return cachedDisplayName; }
    ...
    public boolean isUnsupportedWorldType() { return getUnsupportedWorldType() != null; }
    ...
    public WorldType getUnsupportedWorldType() {
        ...
        for (WorldType worldType : COPILOT_UNSUPPORTED_WORLDS) {
            if (worldTypes.contains(worldType)) { return worldType; }
        }
        return null;
    }
    ...
    public boolean hasJustLoggedIn() { return client.getTickCount() <= lastLoginTick + GE_LOGIN_BURST_WINDOW; }
}

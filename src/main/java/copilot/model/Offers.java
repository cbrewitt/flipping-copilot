...
public class Offers {
    ...
    public synchronized SavedOffer loadOffer(Long accountHash, Integer slot) {
        var slotToOffer = cachedOffers.computeIfAbsent(accountHash, (k) -> new HashMap<>());
    ...
    private void saveAsync(Long accountHash, Integer slot) { executor.submit(() -> save(accountHash, slot)); }

    public synchronized void saveAll() {
        for(Long accountHash: cachedOffers.keySet()) {
            for(Integer slot: cachedOffers.get(accountHash).keySet()) { save(accountHash, slot); }
        }
    }
    ...
    private void save(Long accountHash, Integer slot) {
        ...
        synchronized (file) {
            var offer = loadOffer(accountHash, slot);
            var slotToLastSaved = lastSaved.computeIfAbsent(accountHash, (k)->new HashMap<>());
            var lastSaved = slotToLastSaved.get(slot);
    ...
    private File getFile(Long accountHash, Integer slot) {
        var slotToFile = files.computeIfAbsent(accountHash, (k) -> new HashMap<>());

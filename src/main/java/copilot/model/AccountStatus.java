...
public class AccountStatus {
    ...
    public synchronized boolean isCollectNeeded(Suggestion suggestion, boolean setUpOfferOpen) {
        if (!suggestion.isDumpAlert && !setUpOfferOpen
                && SuggestionType.WAIT.equals(suggestion.type) && offers.reservedSlotNeeded(isWorldMember || isAccountMember, resolveReservedSlots(), suggestion))  {
        ...
        if (suggestion.isModifySuggestion() || suggestion.isAbortSuggestion()) { return false; }
    ...
    public synchronized boolean hasSufficientInventoryForSellSuggestion(Suggestion suggestion) {
        return suggestion != null
                ...
                && inventory != null
                && inventory.getTotalAmount(suggestion.itemId) >= suggestion.quantity;
    }

    public synchronized boolean shouldSellFromBank(Suggestion suggestion) {
        if (suggestion == null || !suggestion.isSellSuggestion() || suggestion.isModifySuggestion()) { return false; }
        if (inventory == null || hasSufficientInventoryForSellSuggestion(suggestion)) { return false; }
        return getBankQuantity(suggestion) > 0;
    }

    private long getBankQuantity(Suggestion suggestion) {
        if (bankAvailable && bankInventory != null) {
            return Math.max(0, bankInventory.getOrDefault(suggestion.itemId, 0));
        }
        if (suggestion.bankItems == null) { return 0; }
        return Math.max(0, suggestion.bankItems.getOrDefault(suggestion.itemId, 0));
    }

    private boolean hasSufficientItemsForSuggestion(Suggestion suggestion) {
        if (!suggestion.isSellSuggestion()) { return true; }

        if (hasSufficientInventoryForSellSuggestion(suggestion)) { return true; }

        if (!bankAvailable || bankInventory == null) { return false; }

        long inventoryQty = inventory.getTotalAmount(suggestion.itemId);
        long bankQty = Math.max(0, bankInventory.getOrDefault(suggestion.itemId, 0));
        return inventoryQty + bankQty >= suggestion.quantity;
    }

    private boolean hasSufficientCollectibleItemsForSuggestion(Suggestion suggestion) {
        if (suggestion == null || !suggestion.isSellSuggestion() || uncollected == null) { return false; }

        long inventoryQty = inventory == null ? 0 : inventory.getTotalAmount(suggestion.itemId);
        long bankQty = 0;
        if (bankAvailable && bankInventory != null) {
            bankQty = Math.max(0, bankInventory.getOrDefault(suggestion.itemId, 0));
        }

        long collectibleQty = Math.max(0, uncollected.getOrDefault(suggestion.itemId, 0L));
        long missingQty = suggestion.quantity - inventoryQty - bankQty;
        return missingQty > 0 && collectibleQty >= missingQty;
    }

    public int findEmptySlot() { return getOffers().findEmptySlot(isWorldMember || isAccountMember); }

    private Map<Integer, Long> computeInventory() {
        var itemAmounts = inventory.getItemAmounts();
    ...
    public synchronized boolean emptySlotExists() { return offers.emptySlotExists(isWorldMember || isAccountMember); }

    private long getTotalGp() { return inventory.getTotalGp(); }

    public synchronized boolean currentlyFlipping() { return offers.stream().anyMatch(Offer::isActive); }
    ...
    private Set<SuggestionType> resolveRequestedSuggestionTypes(boolean geOpen) {
        var requestedSuggestionTypes = SuggestionType.abortAndModifyTypes();
        requestedSuggestionTypes.add(SuggestionType.SELL);
        if (geOpen && !sellOnlyMode) { requestedSuggestionTypes.add(SuggestionType.BUY); }
        return requestedSuggestionTypes;
    }

    private int resolveReservedSlots() { return reservedSlots == null ? 0 : reservedSlots; }
    ...
    public synchronized byte[] encodeProto( boolean geOpen, boolean sendGraphData) {
        return encodeMessage(out -> {
            ...
            if (displayName != null && !displayName.isEmpty()) { out.writeString(1, displayName); }
            ...
            if (offers != null) {
                for (Offer offer : offers) {
                    ...
                    if (offerBytes.length == 0) { continue; }
                    writeDelimitedMessageField(out, 6, offerBytes);
                }
            }

            var protoInventory = computeInventory();
            writeMap(out, 7, protoInventory, CodedOutputStream::writeInt32, CodedOutputStream::writeInt64);

            out.writeBool(11, f2pOnlyMode);
            writePacked(out, 12, blockedItems, CodedOutputStream::writeInt32NoTag);

            var requestedSuggestionTypes = resolveRequestedSuggestionTypes(geOpen);
            List<Integer> requestedSuggestionTypeInts = new ArrayList<>(requestedSuggestionTypes.size());
            for (SuggestionType suggestionType : requestedSuggestionTypes) {
                requestedSuggestionTypeInts.add(suggestionType.protoInt());
            }
            writePacked(out, 13, requestedSuggestionTypeInts, CodedOutputStream::writeInt32NoTag);
            ...
            if (reservedSlots != null) { out.writeInt32(17, reservedSlots); }
            if (minPredictedProfit != null) { out.writeInt64(34, minPredictedProfit); }
            if (dumpMinPredictedProfit != null) { out.writeInt64(35, dumpMinPredictedProfit); }

            writeMap(out, 20, bankInventory, CodedOutputStream::writeInt32, CodedOutputStream::writeInt32 );
            out.writeBool(21, bankAvailable);

            if (suggestionsPaused != null) { out.writeBool(28, suggestionsPaused); }
            out.writeBool(30, buyAndHold);
            writePacked(out, 31, syncExcluded, CodedOutputStream::writeInt32NoTag);
            out.writeBool(32, allowedSync);
            writeMap(out, 33, bagInventory, CodedOutputStream::writeInt32, CodedOutputStream::writeInt32 );
        });
    }
}

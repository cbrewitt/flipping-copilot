...
public class ReactiveStateImpl<T> implements ReactiveState<T> {
    ...
    @Override
    public void update(Function<T, T> updateFunc) { setAndPublish(updateFunc.apply(get())); }

    @Override
    public void set(T newState) {
        if (!Objects.equals(newState, get())) { setAndPublish(newState); }
    }

    public void forceSet(T newState) { setAndPublish(newState); }

    @Override
    public T get() { return state; }

    private void setAndPublish(T value) {
        synchronized (this) { state = value; }

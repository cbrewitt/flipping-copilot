...
public class GeHistoryStateRS extends ReactiveStateImpl<GeHistoryState> {
    ...
    private static final int GE_HISTORY_GROUP = 383, GE_HISTORY_LIST_CHILD = 3;
    ...
    @Inject
    public GeHistoryStateRS(GameLogin gameLogin) {
    ...
    public void onGameTick(Client client) {
        ...
        if (visible && !lastSeenVisible) { sessionOpenedAt = (int) Instant.now().getEpochSecond(); }
        lastSeenVisible = visible;
        if (!visible) { return; }
        var rows = parseRows(container);
        if (rows.isEmpty()) { return; }
        Long accountHash = gameLogin.get().accountHash;
        var current = get();
        if (current.loaded
                && current.capturedAt == sessionOpenedAt
                && Objects.equals(current.accountHash, accountHash)
                && rows.equals(current.rows)) { return; }
    ...
    private static List<GeHistoryRow> parseRows(Widget container) {
        ...
        if (children == null || children.length < 6) { return Collections.emptyList(); }
        List<GeHistoryRow> rows = new ArrayList<>(children.length / 6);
        for (int i = 0; i + 5 < children.length; i += 6) {
            try {
                Widget stateW = children[i + 2], itemW = children[i + 4], priceW = children[i + 5];
                String stateText = stateW.getText();
                if (stateText == null || stateText.isEmpty()) { continue; }
                int itemId = itemW.getItemId(), quantity = itemW.getItemQuantity();
    ...
    private static long parsePrice(String text, int quantity) {
        ...
        if (isTotalPrice && quantity > 0) { return price / quantity; }
        return price;
    }
}

...
public class DumpStream {
    ...
    @Inject
    public DumpStream(ClientThread clientThread,
                                 ApiClient api,
        ...
        this.clientThread = clientThread; this.api = api; this.suggestionController = suggestionController;
        subscribedDisplayName = ReactiveStateUtil.derive(
                copilotLoginRS,
                osrsLoginRS,
                ...
                (copilotLoginState, loginState, preferences, isGrandExchangeOpen) -> {
                    if (!loginState.loggedIn
                            ...
                            || !Boolean.TRUE.equals(isGrandExchangeOpen)) { return null; }
                    return loginState.displayName;
                });
        subscribedDisplayName.registerListener(name -> {
            if (name != null) { consumeDumps(); } else {
                ensureUnsubscribed();
            }
        });
    }
    ...
    private void consumeDumps() {
        ...
        if(displayName != null) {
            ...
            Call call = api.asyncConsumeDumpAlerts(displayName,
    ...
    private void handleDumpMessage(byte[] data) throws IOException {
        ...
        log.info("received dump suggestion {} {}", suggestion.name, suggestion.type);
    }

...
public class Items {
    ...
    public static final int COINS_ITEM_ID = 995, PLATINUM_TOKENS_ITEM_ID = 13204;
    // dependencies
    ...
    @Inject
    public Items(Client client,
                          ClientThread clientThread,
                          ItemManager itemManager,
                          FuzzySearchScorer fuzzySearchScorer,
                          ScheduledExecutorService executorService) {
        this.fuzzySearchScorer = fuzzySearchScorer; this.client = client; this.itemManager = itemManager;
    ...
    public List<ItemIdName> search(String input, Set<Integer> existingSelectedItems) {
        ...
        var comparator = fuzzySearchScorer.comparator(input);
        return cachedItems.stream()
    ...
    public Set<Integer> allItemIds() { return cachedItemNames.keySet(); }

    public String getItemName(Integer itemId) { return cachedItemNames.getOrDefault(itemId, "Name unavailable"); }
    ...
    public long totalCash(Map<Integer, Integer> itemQuantities) {
        if (itemQuantities == null) { return 0L; }
        return itemQuantities.getOrDefault(COINS_ITEM_ID, 0)
                + (long) itemQuantities.getOrDefault(PLATINUM_TOKENS_ITEM_ID, 0) * 1000L;
    }

    public Map<Integer, Integer> getRunliteInventory() { return buildRuneliteItemMap(InventoryID.INVENTORY); }

    public Map<Integer, Integer> getRunliteBankInventory() { return buildRuneliteItemMap(InventoryID.BANK); }

    private Map<Integer, Integer> buildRuneliteItemMap(InventoryID inventoryID) {
        var container = client.getItemContainer(inventoryID);
        ...
        for (Item item : container.getItems()) {
            ...
            if (unnotedTradeableItemId == null) { continue; }
    ...
    private Integer toUnnotedTradeableItemId(int itemId) {
        ...
        if (unnotedItemId == COINS_ITEM_ID || unnotedItemId == PLATINUM_TOKENS_ITEM_ID) { return unnotedItemId; }
        var unnoted = itemManager.getItemComposition(unnotedItemId);
        if (unnoted == null || !unnoted.isGeTradeable()) { return null; }

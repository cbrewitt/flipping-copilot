...
public class HighlightController {
    ...
    private static final int BANK_TAG_TAB_CHILD_OFFSET = 4, CLOSE_BUTTON_INDEX = 11;
    private static final Rectangle CLOSE_BUTTON_HIGHLIGHT_BOUNDS = new Rectangle(2, 2, 19, 19);
    private static final String GE_CLERK_NAME = "Grand Exchange Clerk", BANKER_NAME = "Banker";
    ...
    private final HighlightColorController colors;
    ...
    public void deactivateAndRemoveAll() {
        ...
        try {
            invokeAndWait(clearTask);
        } catch (Exception e) {
            invokeLater(clearTask);
        }
    }
    ...
    public void redraw() {
        ...
        removeAll();
        if (!active) { return; }
        if (!config.suggestionHighlights()) { return; }
        if (offerManager.isOfferJustPlaced()) { return; }
        if (suggestions.getSuggestionError() != null) { return; }
        ...
        if (goToBank && grandExchange.isOpen() && highlightGrandExchangeCloseButton(suggestion)) { return; }
        if (!goToBank && isBankOpen() && !suggestion.isWaitSuggestion() && highlightBankCloseButton(suggestion)) {
            return;
        }
        if (goToBank && drawSellFromBankHighlight(suggestion)) { return; }
        if(!grandExchange.isOpen()) {
            if (!isBankOpen()) { highlightNpcAtGrandExchange(suggestion, accountStatus, goToBank); }
            return;
        }
        if (grandExchange.isHomeScreenOpen()) { drawHomeScreenHighLights(suggestion); } else if (grandExchange.isSlotOpen()) {
    ...
    private void highlightNpcAtGrandExchange(Suggestion suggestion, AccountStatus accountStatus, boolean goToBank) {
        ...
        NPC target;
        if (goToBank) { target = findClosestNpcByName(BANKER_NAME); } else if (drawHomeScreenHighLights(suggestion)) {
        ...
        addNpcHighlight(target, () -> {
            Color base = colors.getBlueColor(flashHighlight);
    ...
    private NPC findClosestNpcByName(String name) {
        ...
        for (NPC npc : client.getNpcs()) {
            if (npc == null || !name.equals(npc.getName())) { continue; }
    ...
    private void addNpcHighlight(NPC npc, Supplier<Color> colorSupplier) {
        if (npc != null) enqueueOverlay(() -> new NpcHighlightOverlay(npc, colorSupplier, modelOutlineRenderer));
    }

    private boolean drawHomeScreenHighLights(Suggestion suggestion) {
        boolean flashHighlight = suggestion.isBuyDumpSuggestion();
        Supplier<Color> blueHighlight = () -> colors.getBlueColor(flashHighlight);
        Supplier<Color> redHighlight = () -> colors.getRedColor(flashHighlight);
        Supplier<Color> amberHighlight = () -> colors.getAmberColor(flashHighlight);
        var accountStatus = accounts.getAccountStatus();
        if (accountStatus.isCollectNeeded(suggestion, grandExchange.isSetupOfferOpen())) {
            ...
            if (collectButton != null) { add(collectButton, blueHighlight, new Rectangle(2, 1, 81, 18)); }
            return true;
        }
            ...
            Widget slotWidget = grandExchange.getSlotWidget(suggestion.boxId);
            if (slotWidget != null && !slotWidget.isHidden()) { add(slotWidget, amberHighlight); }
            return true;
        }
        else if (isScanningForDumpsSuggested(suggestion, accountStatus) || suggestion.isBuySuggestion()) {
            ...
            Widget itemWidget = getInventoryItemWidget(suggestion.itemId);
    ...
    private boolean drawSellFromBankHighlight(Suggestion suggestion) {
        ...
        Widget bankItemWidget = getBankItemWidget(suggestion.itemId);
    ...
    private boolean highlightCloseButton(Widget closeButton, Suggestion suggestion) {
        if (closeButton == null || closeButton.isHidden()) { return false; }
    ...
    private void highlightCreateBuyOfferButton(AccountStatus accountStatus, Supplier<Color> colorSupplier) {
        ...
        if (buyButton != null && !buyButton.isHidden()) { add(buyButton, colorSupplier, new Rectangle(0, 0, 45, 44)); }
    }

    private void drawOfferScreenHighlights(Suggestion suggestion) {
        boolean flashHighlight = suggestion.isBuyDumpSuggestion();
        Supplier<Color> blueHighlight = () -> colors.getBlueColor(flashHighlight);
        var s = grandExchange.getOfferScreenSetupOfferState();
        if (s == null) { return; }
        ...
        boolean itemMatches = s.currentItemId == suggestion.itemId;

        // Prioritise certain buy dump alert cases
        if (suggestion.isBuyDumpSuggestion()) {
            if (!offerTypeMatches || accounts.getAccountStatus().isCollectNeeded(suggestion, grandExchange.isSetupOfferOpen())) {
                ...
                if (s.offerPrice != suggestion.price) { highlightPrice(blueHighlight); }
        ...
        if (isCustomChoiceItem(s, offerTypeMatches, itemMatches)) {
            if (s.offerPrice == offerManager.getViewedSlotItemPrice()) { highlightConfirm(blueHighlight); } else {
    ...
    private boolean isCustomChoiceItem(OfferSetup s, boolean offerTypeMatches, boolean itemMatches) {
    ...
    private void highlightItemInSearch(Suggestion suggestion, Supplier<Color> colorSupplier) {
        if (!client.getVarcStrValue(VarClientStr.INPUT_TEXT).isEmpty()) { return; }
        ...
        for (Widget widget : searchResults.getDynamicChildren()) {
            if (widget.getName().equals("<col=ff9040>" + suggestion.name + "</col>")) {
                add(widget, colorSupplier);
                return;
            }
        }
        Widget itemWidget = searchResults.getChild(3);
        if (itemWidget != null && itemWidget.getItemId() == suggestion.itemId) { add(itemWidget, colorSupplier); }
    }

    private void highlightPrice(Supplier<Color> colorSupplier) {
        add(grandExchange.getSetPriceButton(), colorSupplier, new Rectangle(1, 6, 33, 23));
    }

    private void highlightQuantity(Suggestion suggestion, int offerQuantity, Supplier<Color> colorSupplier) {
        var accountStatus = accounts.getAccountStatus();
        if (offerQuantity != suggestion.quantity) {
            Widget setQuantityButton;
            if (accountStatus.getInventory().getTotalAmount(suggestion.itemId) == suggestion.quantity
                && suggestion.isSellSuggestion()) { setQuantityButton = grandExchange.getSetQuantityAllButton(); } else {
                setQuantityButton = grandExchange.getSetQuantityButton();
            }
            if (setQuantityButton != null) { add(setQuantityButton, colorSupplier, new Rectangle(1, 6, 33, 23)); }
        }
    }

    private void highlightConfirm(Supplier<Color> colorSupplier) {
        add(grandExchange.getConfirmButton(), colorSupplier, new Rectangle(1, 1, 150, 38));
    }

    private Widget getItemSearchButtonWidget() {
        Widget setupContainer = client.getWidget(465, 26);
        if (setupContainer == null) { return null; }
    ...
    private void highlightItemSearchButton(Supplier<Color> colorSupplier) {
        add(getItemSearchButtonWidget(), colorSupplier);
    }

    private void highlightBackButton(Supplier<Color> colorSupplier) {
        add(grandExchange.getBackButton(), colorSupplier);
    }

    private void add(Widget widget, Supplier<Color> colorSupplier, Rectangle adjustedBounds) {
        if (widget != null) enqueueOverlay(() -> new WidgetHighlightOverlay(widget, colorSupplier, adjustedBounds));
    }

    private void enqueueOverlay(Supplier<Overlay> factory) {
        if (!active) return;
        final int addGeneration = generation.get();
        invokeLater(() -> {
            if (!active || generation.get() != addGeneration) return;
            Overlay overlay = factory.get();
    ...
    private Widget getInventoryItemWidget(int unnotedItemId) {
        ...
        Widget notedWidget = null, unnotedWidget = null;
        ...
        for (Widget widget : children) {
            ...
            if (itemComposition.getNote() != -1) {
                if (itemComposition.getLinkedNoteId() == unnotedItemId) { notedWidget = widget; }
    ...
    private Widget getBankItemWidget(int unnotedItemId) {
        for (int childId : BANK_ITEM_CONTAINER_CHILDREN) {
            ...
            if (bankItems == null || bankItems.isHidden()) { continue; }

            Widget itemWidget = getVisibleItemWidget(bankItems, unnotedItemId);
            if (itemWidget != null) { return itemWidget; }
        }
        return null;
    }
    ...
    private Widget getBankCloseButton() { return getCloseButton(InterfaceID.Bankmain.FRAME); }

    private Widget getGrandExchangeCloseButton() { return getCloseButton(InterfaceID.GeOffers.FRAME); }

    private Widget getCloseButton(int frameId) {
        Widget frame = client.getWidget(frameId);
        if (frame == null || frame.getDynamicChildren() == null) return null;
        Widget[] children = frame.getDynamicChildren();
        return children.length <= CLOSE_BUTTON_INDEX ? null : children[CLOSE_BUTTON_INDEX];
    }

    private Widget getPortfolioBankTagButton() {
        if (!config.portfolioBankTag()
                || !pluginManager.isPluginActive(bankTagsPlugin)
                || PORTFOLIO_BANK_TAG.equals(bankTagsService.getActiveTag())) { return null; }
        ...
        for (int i = BANK_TAG_TAB_CHILD_OFFSET; i < children.length; i += 2) {
            ...
            if (PORTFOLIO_BANK_TAG.equals(Text.removeTags(widgetName))) { return button; }
        }
        return null;
    }
    ...
    private Widget getVisibleItemWidget(Widget itemContainer, int unnotedItemId) {
        ...
        for (Widget widget : children) {
            if (widget == null || widget.isHidden() || widget.getItemQuantity() <= 0) { continue; }

            if (!matchesItemId(widget.getItemId(), unnotedItemId)) { continue; }

            Rectangle bounds = widget.getBounds();
            if (clipToContainerBounds && (bounds == null || !containerBounds.intersects(bounds))) { continue; }
    ...
    private boolean matchesItemId(int itemId, int unnotedItemId) {
        if (itemId == unnotedItemId) { return true; }

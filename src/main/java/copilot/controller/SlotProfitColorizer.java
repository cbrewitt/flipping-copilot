...
public class SlotProfitColorizer {
    ...
    public void updateAllSlots() {
        if (!config.slotPriceColorEnabled()) { return; }

        int openSlot = grandExchange.getOpenSlot();
        if (openSlot > -1) {
            if (grandExchange.isSetupOfferOpen()) { updateOfferSetup(); } else {
    ...
    private void updateOverviewSlot(int slotIndex) {
        try {
            Widget slotWidget = getSlotWidget(slotIndex), priceTextWidget = getPriceTextWidget(slotWidget);
    ...
    private void updatePriceTextColor(Widget priceTextWidget, int slotIndex, Color defaultColor) {
        if (!hasValidText(priceTextWidget)) { return; }
    ...
    private void updateOfferSetup() {
        try {
            ...
            if (offerSetupWidget == null || offerSetupWidget.isHidden()) { return; }

            Widget priceTextWidget = offerSetupWidget.getChild(OFFER_SETUP_PRICE_TEXT_CHILD_INDEX);
            if (priceTextWidget == null) { return; }
    ...
    private Color determineOfferSetupColor(Color defaultColor) {
        ...
        if (itemId <= 0) { return defaultColor; }
    ...
    private Color determineProfitColor(Long profit, Color defaultColor) {
        if (profit == null || profit == 0) { return defaultColor; }

        if (profit < 0) { return config.slotPriceUnprofitableColor(); } else {
            return config.slotPriceProfitableColor();
        }
    }
    ...
    public void resetOfferSetup() {
        try {
            Widget offerSetupWidget = client.getWidget(InterfaceID.GE_OFFERS, OFFER_SETUP_CHILD_ID);
            if (offerSetupWidget == null) { return; }
    ...
    private boolean isBuyOffer(int slotIndex) {
        try {
            ...
            var savedOffer = getOffer(slotIndex);
            if (savedOffer != null) { return savedOffer.getOfferStatus() == OfferStatus.BUY; }
            ...
            if (offers != null && slotIndex < offers.length) {
                ...
                if (currentOffer != null) {
                    var state = currentOffer.getState();
    ...
    private boolean isOfferTracked(int slotIndex) { return getOffer(slotIndex) != null; }

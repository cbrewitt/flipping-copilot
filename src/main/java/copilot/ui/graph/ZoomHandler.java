...
public class ZoomHandler {
    ...
    public void drawButtons(Graphics2D graphics, Rectangle pa, Point p) {
        ...
        Rectangle[] buttons = {homeButtonRect, maxButtonRect, zoomInButtonRect, zoomOutButtonRect};
        for (int i = 0; i < buttons.length; i++) {
            Rectangle button = buttons[i];
            if (i > 0) x -= size + GRAPH_BUTTON_MARGIN;
            drawButtonBackground(graphics, button, x, y, size, isOver(button, p));
            switch (i) {
                case 0: drawHomeIcon(graphics, button); break;
                case 1: drawMaxIcon(graphics, button); break;
                default: drawPlusMinusIcon(graphics, button, i == 2);
            }
        }

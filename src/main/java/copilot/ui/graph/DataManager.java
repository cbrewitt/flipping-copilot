...
public class DataManager {
    ...
    public final List<Datapoint> highDatapoints = new ArrayList<>(), lowDatapoints = new ArrayList<>();
    public final List<Datapoint> predictionLowDatapoints = new ArrayList<>();
    public final List<Datapoint> predictionHighDatapoints = new ArrayList<>(), volumes = new ArrayList<>();
    ...
    public Datapoint findClosestPoint(Point mousePos, int hoverRadius, Rectangle pa, Bounds bounds) {
        ...
        var closest = closestWithin(mousePos, hoverRadius * 2, pa, bounds,
    ...
    public Bounds calculateHomeBounds() {
        ...
        int range = xMax - xMin;
        if(!flipEntryDatapoints.isEmpty()) { xMin -= Math.max(range / 10, Constants.DAY_SECONDS); }
        if(!flipCloseDatapoints.isEmpty()) {
            xMax += Math.max(range / 10, Constants.DAY_SECONDS);
            xMax = Math.min(xMax, maxBounds.xMax);
        }
        int finalXMax = xMax, finalXMin = xMin;
    ...
    public Bounds calculateBounds(Predicate<Datapoint> p) {
        ...
        b.xMin = Integer.MAX_VALUE; b.xMax = Integer.MIN_VALUE; b.yMax = Long.MIN_VALUE; b.yMin = Long.MAX_VALUE;
        b.y2Min = 0; b.y2Max = 10;
        ...
        for(List<Datapoint> datapoints : Arrays.asList(highDatapoints, lowDatapoints, predictionLowDatapoints, predictionHighDatapoints, flipEntryDatapoints, flipCloseDatapoints)) {
            for (Datapoint d : datapoints) {
                if (p.test(d)) {
                    ...
                    b.xMin = Math.min(b.xMin, d.time); b.xMax = Math.max(b.xMax, d.time);

                    // Update price bounds
                    b.yMin = Math.min(b.yMin, d.price); b.yMax = Math.max(b.yMax, d.price);

                    // Check IQR bounds for prediction points
                    if (d.type == Datapoint.Type.PREDICTION) {
                        if (d.iqrLower != null) { b.yMin = Math.min(b.yMin, d.iqrLower); }
                        if (d.iqrUpper != null) { b.yMax = Math.max(b.yMax, d.iqrUpper); }
                    }
                }
            }
        }
        for (Datapoint d : volumes) {
            if (p.test(d)) { b.y2Max = Math.max(b.y2Max, d.highVolume + d.lowVolume); }
    ...
    private void addHistoricalPoints(List<Datapoint> target,
        ...
        for (int i = times.length - 1; i >= 0; i--) {
            if (times[i] < cut) { target.add(0, new Datapoint(times[i], prices[i], isLow, type)); }
        }
    }
    ...
    private void processDatapoints() {
        ...
        if (data.lowLatestTimes == null) { return; }
        ...
        if(fpr != null){
            if(fpr.buyTimes != null) {
                for (int i = 0; i < fpr.buyTimes.length; i++) {
                    flipEntryDatapoints.add(Datapoint.newBuyTx(fpr.buyTimes[i], fpr.buyPrices[i], fpr.buyVolumes[i] ));
        ...
        roundToHours(maxBounds);
    }

    private void roundToHours(Bounds bounds) {
        bounds.xMin = (bounds.xMin / Constants.HOUR_SECONDS) * Constants.HOUR_SECONDS;
        bounds.xMax = (bounds.xMax / Constants.HOUR_SECONDS) * Constants.HOUR_SECONDS + Constants.HOUR_SECONDS;
    }
    ...
    private void calculateStats() {
        ...
        if (!lowDatapoints.isEmpty() && !highDatapoints.isEmpty()){
            ...
            priceChange24H = priceChangeSince(cut24h, currentLowPrice, currentHighPrice);
            priceChangeWeek = priceChangeSince(cutWeek, currentLowPrice, currentHighPrice);
        }

...
public class GraphRenderer {
    ...
    private final Rectangle pa;
    private final Bounds bounds;


    public void drawGrid(BiFunction<Rectangle, Long, Integer> toY, TimeAxis xAxis, YAxis yAxis) {
        ...
        for (int[] times : new int[][]{xAxis.timeOnlyTickTimes, xAxis.gridOnlyTickTimes}) {
            for (int time : times) {
                int x = bounds.toX(pa, time);
                graphics.drawLine(x, pa.y, x, pa.y + pa.height);
            }
        }
        for (long[] values : new long[][]{yAxis.tickValues, yAxis.gridOnlyValues}) {
            for (long value : values) {
                int y = toY.apply(pa, value);
    ...
    public void drawXAxisLabels(TimeAxis xAxis) {
    ...
    public void drawYAxisLabels(BiFunction<Rectangle, Long, Integer> toY, YAxis yAxis, boolean skipLast) {
        graphics.setFont(graphics.getFont().deriveFont(FONT_SIZE)); graphics.setColor(config.textColor);
        var metrics = graphics.getFontMetrics();
    ...
    public void drawPredictionIQR(int[] times, long[] lowerPrices, long[] upperPrices, boolean isLow) {
        if (times.length < 2) return;

        // Set appropriate color
        graphics.setColor(isLow ? config.lowShadeColor : config.highShadeColor);

        // Create path for shaded pa with clipping
    ...
    public void drawLines(List<Datapoint> datapoints, Color color, Stroke stroke) {
    ...
    public void drawStartPoints(List<Datapoint> points, Color color, int size) {
        drawPoints(points, color, size, true);
    }

    public void drawPoints(List<Datapoint> points, Color color, int size) { drawPoints(points, color, size, false); }

    private void drawPoints(List<Datapoint> points, Color color, int size, boolean starts) {
        ...
        for (Datapoint point : points) {
            ...
            int x = bounds.toX(pa, point.time), y = bounds.toY(pa, point.price);
            int half = size / 2;
            if (starts) {
                ...
                graphics.fillRect(x, y, bounds.toW(pa, duration) + size, size);
            }
        }
        graphics.setClip(originalClip);
    }

    public void drawLegend(boolean predictions) {
    ...
    public void drawVolumeBars(List<Datapoint> volumes, Datapoint hoveredPoint) {
        var originalClip = graphics.getClip();
        graphics.setClip(pa.x, pa.y, pa.width, pa.height);
        ...
        for (Datapoint v : volumes) {
            ...
            boolean hovered = hoveredPoint == v;
            graphics.setColor(hovered ? Color.WHITE : Color.GRAY);
            graphics.setStroke(hovered ? THICK_STROKE : THIN_STROKE);
            graphics.drawRect(x1, y1, x2 - x1, y3 - y1);
        }
        graphics.setClip(originalClip); // restore original clip
    }

    public void drawTxsDatapoints(List<Datapoint> txDatapoints, Datapoint hoveredPoint) {

...
public final class PatchNotesPopup {
    ...
    private static final int LOGO_WIDTH = 50, LOGO_HEIGHT = 45;
    ...
    public static void show(Component parent) {
        var heading = UIUtilities.coloredLabel("Flipping Copilot has been updated to v1.8.0", Color.WHITE); heading.setFont(heading.getFont().deriveFont(Font.BOLD, 16f));

        var notes = new JTextPane();
        notes.setEditable(false); notes.setFocusable(false); notes.setForeground(Color.WHITE);
        notes.setBackground(DARKER_GRAY_COLOR); notes.setFont(UIManager.getFont("TextArea.font"));
        notes.setBorder(BorderFactory.createEmptyBorder(8, 8, 8, 8));
        writeNotes(notes);

        var scrollPane = new JScrollPane(notes);
        scrollPane.setPreferredSize(new Dimension(430, 220));
        scrollPane.setBorder(BorderFactory.createLineBorder(DARK_GRAY_COLOR));
        scrollPane.getViewport().setBackground(DARKER_GRAY_COLOR);

        var panel = UIUtilities.darkPanel(new BorderLayout(0, 10), DARKER_GRAY_COLOR);
    ...
    private static void writeNotes(JTextPane pane) {
        ...
        try {
            doc.insertString(doc.getLength(), "Two new features in this update:\n\n", null);
    ...
    private static Icon buildLogoIcon() {
        var logo = ImageUtil.loadImageResource(PatchNotesPopup.class, "/logo.png");

...
public class StatsPanelV2 extends JPanel {
    ...
    private JPanel buildSubInfoPanel() {
        ...
        subInfoPanel.add(buildSubInfoPanelItem("Avg cash stack:", avgCashVal, ColorScheme.LIGHT_GRAY_COLOR));
    ...
    private void setupProfitAndSubInfoPanel() {
        ...
        // Initially hide the sub-info panel
    ...
    public void updateStatsAndFlips(boolean flipsMaybeChanged) {
        ...
        if (flipsMaybeChanged) {
            ...
            totalProfitVal.setForeground(stats.profit >= 0 ? ColorScheme.GRAND_EXCHANGE_PRICE : UIUtilities.getProfitColor(stats.profit, config));
            log.debug("populating flips took {}ms", (System.nanoTime() - s) / 1000_000);
        }

        // 'Session time', 'Hourly profit' and 'Avg cash stack' should only be set if 'Session' is select in the dropdown
        if (IntervalTimeUnit.SESSION.equals(selectedIntervalTimeUnit)) {

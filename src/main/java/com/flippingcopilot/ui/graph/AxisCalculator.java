...
public class AxisCalculator {
    ...
    public static TimeAxis calculateTimeAxis(PlotArea pa, int localTimeOffsetSeconds) {
        ...
        int days = pa.bounds.xDelta() / DAY_SECONDS;
        int daysStep = Math.max(1, days / 7);

        int maxDay = ((timeMax + localTimeOffsetSeconds) / DAY_SECONDS) * DAY_SECONDS - localTimeOffsetSeconds;
        int minDay = ((timeMin + localTimeOffsetSeconds) / DAY_SECONDS) * DAY_SECONDS - localTimeOffsetSeconds;
        int[] dayTicks = new int[]{};
        while (maxDay > timeMin) {
            dayTicks = append(dayTicks, maxDay);
            maxDay -= daysStep * DAY_SECONDS;
        }
        ...
        if (daysStep == 1) {
            if (dayTicks.length < 5) {
                ...
                // add only 12:00
                for (int d : preAppend(dayTicks, dayTicks[0] - DAY_SECONDS)) {
                    int tick12 = d + 12 * 60 * 60;    // 12:00

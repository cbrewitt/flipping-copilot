...
public class FlipFilterAndSort {
    ...
    private synchronized void _reloadFlips(boolean totalPagesMaybeChanged, boolean forceReload) {
        try {
            if (canUseFlipsManager()) {
                ...
                flipsCallback.accept(flipManager.getPageFlips(page, pageSize, intervalStartTime, accountId, includeBuyingFlips));
            } else {
    ...
    private String formatTimestampISO(int timestamp) {
        if (timestamp == 0) {
            return "";
        }
